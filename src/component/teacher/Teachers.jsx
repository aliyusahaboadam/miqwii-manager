...
import { deleteTeacherById, getAllTeachers, getTeacherCount } from '../../redux/reducer/teacherSlice';
...
const Teachers  = () => {
  ...
  const {teachers, fetchingStatus, teachersCount} = teacherState;
...
localStorage.setItem('authenticated', JSON.stringify(authenticated));
  ...
  // const [rows, setRows] = useState(classData);
      ...
      const handleDelete = async (id)  => {
        ...
        const result = await dispatch(deleteTeacherById(id)).unwrap();
      };
    
      const handleEdit = (id) => {
        // Implement edit functionality
        navigate(`/teacher/update-teacher/${id}`);
      };


      const handleViewDetails = (id) => {
        // Implement edit functionality
        navigate(`/teacher/teacher-details/${id}`);
      };
...
<div className={dashboard['secondary--container']}>
      ...
      <div class={[dashboard['grid'], dashboard['grid--1x2']].join(' ')}>
                ...
                <span class={[dashboard['badge'], dashboard['']].join(' ')}>{teachersCount}</span>
                
                
                </div>
                
                
                Total Available  Teachers
                ...
                <div class={dashboard['card_body']}>
                    ...
                    <button onClick={navigateToAddTeacher}  className={[dashboard['btn'], dashboard['btn--block'], dashboard['btn--primary']].join(' ')}>Add Teacher</button>
                        ...
                        : rows
                          ...
                          <StyledTableRow key={row.id}>
                            ...
                            <StyledTableCell component="th" scope="row">
                             ...
                             <StyledTableCell component="th" scope="row">
                              {
                              row?.classes?.length > 0 ? row.classes
        .map(cls => cls?.name)
        .filter(Boolean)
        .join(', ') : 'No Class'
                            }
                            </StyledTableCell>
                            <StyledTableCell component="th" align="left">{row.firstname + ' ' + row.surname + ' ' + row.lastname}</StyledTableCell>
