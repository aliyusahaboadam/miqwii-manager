...
const StudentResultByRegNo = ({ requestData }) => {
  ...
  const generatePDF = async () => {
    ...
    const resultAction = await dispatch(getResultByStudentRegNo(regNoData));
 
    if (!getResultByStudentRegNo.fulfilled.match(resultAction)) {
    ...
    return {
      blob,
      studentName: fetchedResult.student?.firstname,
      regNo: fetchedResult.student?.regNo,
    };
  };
  ...
  const handleDownloadPDF = async () => {
    try {
      ...
      const { blob, studentName, regNo } = await generatePDF();
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = `result-${studentName || regNo || 'student'}.pdf`;
