...
import { getStudentById, updateStudent } from '../../redux/reducer/studentSlice';
...
const [state, setState ] = useState({
  ...
  useEffect(() => {
       ...
       const   fetchStudent = async () => {
        try {
          setIsLoading(true)
          await  dispatch(getStudentById(id)).unwrap().then((result) => {
...
localStorage.setItem('authenticated', JSON.stringify(authenticated));
     ...
     const handleFormSubmit = async (values, { resetForm })  => {
        const updatedStudent = ({
           ...
           try {
          const result = await dispatch(updateStudent({id: state.id, studentData: updatedStudent})).unwrap();
...
<FormHelperText sx={{ fontSize: 15 }}>{touched.gender && errors.gender}</FormHelperText>
    ...
    <button  disabled={isSubmitting}  type="submit" onClick={handleSubmit} className={[style['btn'], style['btn--block'], style['btn--primary']].join(' ')}>{isSubmitting ? 'Submitting...' : 'Update Student'}</button>
