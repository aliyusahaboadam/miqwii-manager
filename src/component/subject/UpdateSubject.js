...
import { getSubjectById, updateSubject } from '../../redux/reducer/subjectSlice';
...
const [state, setState ] = useState({
  ...
  useEffect(() => {
      ...
      const   fetchStudent = async () => {
        try {
          setIsLoading(true)
          await  dispatch(getSubjectById(id)).unwrap().then((result) => {
     ...
     const handleFormSubmit = async (values, { resetForm })  => {
        const updatedSubject = ({
            ...
            name: values.name,
              ...
              console.log("Test for student data " + JSON.stringify(updatedSubject));

           try {
          const result = await dispatch(updateSubject({id: state.id, subjectData: updatedSubject})).unwrap();
          setAlertType("success");
          setMessage(result.message);
           navigate(`/subject/subjects/${className}`);
           } catch (error) {
            setAlertType("error");
            setMessage(error.message);
            console.log(error.message);
...
<Formik
                ...
                {({
                 ...
                 <section class={style.container__brand}>
                  ...
                  {/*Card Header*/}
                <p className={style['form-header']}>Update Subject</p>
...
<TextField
    ...
    />
             ...
             <button  disabled={isSubmitting}  type="submit" onClick={handleSubmit} className={[style['btn'], style['btn--block'], style['btn--primary']].join(' ')}>{isSubmitting ? 'Submitting...' : 'Update Subject'}</button>
