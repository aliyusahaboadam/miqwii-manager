...
const handleClose = (event, reason) => {
  ...
  setOpen(false); // Close the Snackbar
     ...
     const handleFormSubmit = async (values, { resetForm })  => {
           console.log(values.class1.name);
           console.log(values.subjects);
           try {
          const resultAction = await dispatch(saveSubject({className: values.class1.name, subjects: values.subjects})).unwrap();
...
const subjects = [
  ...
  { name: "Religious and National Values" },
