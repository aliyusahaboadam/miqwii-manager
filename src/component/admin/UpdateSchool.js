...
const UpdateSchool = () => {
  ...
  const schoolRegistrationSchema = object({
    name: string().max(80, "School Name must not exceed 80 characters")
              .required("School Name is required"),

    address: string().max(80, "Address must not exceed 80 characters")
    .required("Address is required"),
    
    motto: string().max(80, "Motto must not exceed 80 characters")
     ...
     const handleFormSubmit = async (values, { resetForm })  => {
        ...
        const updatedSchool = ({
            ...
            contact: values.contact,
                    ...
                    try {
                      const body = await dispatch(updateSchool({id: state.id, schoolData: updatedSchool})).unwrap();
                      console.log(body);
