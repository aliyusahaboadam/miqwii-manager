...
import { saveReceipt } from '../../redux/reducer/receiptSlice';
import { unCheckAllReceipt } from '../../redux/reducer/receiptSlice';
...
import SchoolFeeSheet from '../result/SchoolFeeSheet';
...
import { getStudentByClassForReceipt } from '../../redux/reducer/receiptSlice';
...
const StudentReceipt  = () => {
  ...
  const receiptState = useSelector((state) => state.receipts);
  const {studentsInClass, fetchingStatus} = receiptState;
 ...
 const [refreshFlag, setRefeshFlag] = useState(true);
...
localStorage.setItem('authenticated', JSON.stringify(authenticated));
   ...
   useEffect(() => {
    ...
    }, [location.pathname, refreshFlag]);

  const fetchData = () => {
   dispatch(getStudentByClassForReceipt(className));
  }
    ...
    const handleCheckBox = (event, studentId) => {
       if (event.target.checked) {
       dispatch(saveReceipt(studentId));
       } else {
        dispatch(saveReceipt(studentId));
       }
       setRefeshFlag(true);
    ...
    const unCheckAllReceiptHandler = () => {
      dispatch(unCheckAllReceipt(className));
    }
...
<div className={dashboard['secondary--container']}>
          ...
          <span class={[dashboard['badge'], dashboard['']].join(' ')}>{paidCount}</span>
          ...
          <span class={[dashboard['badge'], dashboard['']].join(' ')}>{unpaidCount}</span>
          ...
          <div class={dashboard['card_body']}>
              ...
              <button onClick={unCheckAllReceiptHandler}  className={[dashboard['btn'], dashboard['btn--block'], dashboard['btn--primary']].join(' ')}>Uncheck All</button>
...
{/* <div>{classNamesSpecific}</div> */}
            ...
            <TableContainer component={Paper} sx={{ marginTop: 1 }}>
                    <Table sx={{ minWidth: 650 }} aria-label="simple table">
                      <TableHead>
                        <TableRow>
                          ...
                          <StyledTableCell align="right">Tick</StyledTableCell>
                        </TableRow>
                      </TableHead>
                      <TableBody>
                      {(rowsPerPage > 0
                        ...
                        : rows
                          ...
                          <StyledTableRow key={row.id}>
                            ...
                            <StyledTableCell component="th" align="left">{row.fullname}</StyledTableCell>
                            <StyledTableCell component="th" scope="row">
                              {row.feeStatus}
                            </StyledTableCell>
                  ...
                  <FormGroup>
                    ...
                    checked={row.feeStatus === "paid" ? true : false}
                    onChange={(event) => handleCheckBox(event, row.id)}
                     sx={{
                        transform: 'scale(2.5)', // Scale the whole switch
