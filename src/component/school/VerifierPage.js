...
useEffect(() =>  {
  async function fetchData() {
    try {
          const result = await dispatch(getVerificationStatus(resetToken)).unwrap();
   ...
   <SignInContainer>
      ...
      >

        {
          ...
          <div class={dashboard['card_body']}>
                <button   type="submit" onClick={() => navigateToLogin()} className={[dashboard['btn'], dashboard['btn--block'], dashboard['btn--primary']].join(' ')}>{'Proceed to login'}</button>
        </div>
        </div>
        </div>
        ...
        <p class={dashboard['card_footer']}>error</p>
           <div class={[dashboard['card--add'], dashboard['card--primary']].join(' ')}>
          <div class={dashboard['card_body']}>
                <button   type="submit" onClick={() => backToRegister()} className={[dashboard['btn'], dashboard['btn--block'], dashboard['btn--primary']].join(' ')}>{'Back To Register'}</button>
        </div>
        </div>
      </div>
